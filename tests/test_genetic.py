...
from tstrait.genetic import genetic_value
from tstrait.genetic import _GeneticValue
...
class TestInput:
    ...
    def test_input_type(self, sample_ts, sample_df):
        with pytest.raises(
            ...
            genetic_value(ts=1, trait_df=sample_df)
        with pytest.raises(
            TypeError,
            match="trait_df must be a <class 'pandas.core.frame.DataFrame'> instance",
        ):
            genetic_value(ts=sample_ts, trait_df=1)
    ...
    def test_bad_input(self, sample_ts, sample_df):
        with pytest.raises(
            ...
            genetic_value(ts=sample_ts, trait_df=df)
    ...
    def test_no_individual(self, sample_df):
        ...
        with pytest.raises(
            ...
            genetic_value(ts=ts, trait_df=df)
...
class TestOutputDim:
    ...
    def test_output_simple(self, sample_ts, sample_df):
        genetic_result = genetic_value(ts=sample_ts, trait_df=sample_df)
        self.check_dimensions(genetic_result, sample_ts.num_individuals)

    """
    This will be tested after sim_trait is implemented
    ...
    def test_output_sim_trait(self, sample_ts, trait_model):
        num_causal = 10
        trait_df = sim_trait(ts=sample_ts, num_causal=num_causal, model=trait_model)
        genetic_result = genetic_value(ts=sample_ts, trait_df=trait_df)
        self.check_dimensions(genetic_result, sample_ts.num_individuals)
    ...
    def test_output_multivariate(self, sample_ts):
        ...
        trait_df = sim_trait(ts=sample_ts, num_causal=num_causal, model=model)
        genetic_result = genetic_value(ts=sample_ts, trait_df=trait_df)
...
class TestGenotype:
    ...
    def test_binary_tree(self):
        ...
        genetic_result = genetic_value(ts=ts, trait_df=trait_df)
    ...
    def test_allele_freq_one(self):
        ...
        trait_df = pd.DataFrame(
            {"site_id": [4], "effect_size": [1], "trait_id": [0], "causal_state": ["A"]}
        )
        genetic_result = genetic_value(ts=ts, trait_df=trait_df)
        genetic_df = pd.DataFrame(
            {
