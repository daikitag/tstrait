...
from tstrait.trait import _TraitSimulator
...
class TestInput:
    ...
    def test_input_type(self, sample_ts, sample_trait_model):
        with pytest.raises(
            ...
            sim_trait(ts=1, num_causal=1, model=sample_trait_model)
        with pytest.raises(
            TypeError,
            match="model must be a <class 'tstrait.trait_model.TraitModel'> instance",
        ):
            sim_trait(ts=sample_ts, num_causal=1, model="model")
    ...
    def test_bad_input(self, sample_ts, sample_trait_model):
        ...
        with pytest.raises(ValueError, match="No mutation in the tree sequence input"):
            sim_trait(ts=ts, num_causal=1, model=sample_trait_model)
        ...
        with pytest.raises(
            ...
            sim_trait(ts=sample_ts, num_causal=num_causal, model=sample_trait_model)
    ...
    def test_num_causal_input(self, sample_ts, sample_trait_model, num_causal):
        with pytest.raises(TypeError, match="num_causal must be an integer"):
            sim_trait(ts=sample_ts, num_causal=1.0, model=sample_trait_model)
        with pytest.raises(
    ...
    def test_alpha_input(self, sample_ts, sample_trait_model):
        with pytest.raises(TypeError, match="alpha must be numeric"):
            sim_trait(ts=sample_ts, num_causal=1, model=sample_trait_model, alpha="1")
...
class TestOutputDim:
    ...
    def test_output(self, sample_ts, trait_model):
        num_causal = 10
        sim_result = sim_trait(ts=sample_ts, num_causal=num_causal, model=trait_model)
    ...
    def test_output_binary(self, sample_trait_model):
        ...
        sim_result = sim_trait(ts=ts, num_causal=num_causal, model=sample_trait_model)
        self.check_dimensions(sim_result, num_causal)
...
class Test_KSTest:
    ...
    def test_fixed(self, sample_ts):
        ...
        sim_result = sim_trait(ts=sample_ts, num_causal=1000, model=model)
...
class TestGenotype:
    ...
    def test_allele_freq_one(self):
        ...
        value = 2
        fixed_model = tstrait.trait_model(distribution="fixed", value=value)
        trait_df = sim_trait(ts, num_causal=1, model=fixed_model, alpha=0)
    ...
    def test_allele_freq_one_freq_dep(self):
        ...
        trait_df = sim_trait(ts, num_causal=1, model=fixed_model, alpha=-1)
